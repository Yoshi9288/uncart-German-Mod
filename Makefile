...
#---------------------------------------------------------------------------------
ARCH	:=	-mcpu=arm946e-s -mthumb -mthumb-interwork

CFLAGS	:=	-g -Wall -Wextra -O2 -flto\
			-fomit-frame-pointer\
			-ffast-math -std=c11\
			$(ARCH)
...
ASFLAGS	:=	-g $(ARCH)
LDFLAGS	=	-nostartfiles -g -T ../linker.ld $(ARCH)
...
a9lh: common
	...
	@mv $(OUTPUT).bin arm9loaderhax.bin
...
clean:
	@echo clean ...
	@rm -fr $(BUILD) $(OUTPUT).elf arm9loaderhax.bin uncart_brahma.bin
